...
const Leverage = ({ 
    ...
    useEffect(() => {
        setLeverageList(data.responseLeverage);
        setLeverage(data._getTradeOrderComputationData.leverage);
    },[]);
                ...
                <div className={`font-bold ${classNames(styles.value)} ${label.valueColor? label.valueColor : ''}`}>
                    {leverageList? (
                        <select 
                            ...
                            {leverageList.map((res : any, index : number) => (
                                <option id={`leverage-${index}`} value={res.value} key={index}>{res.name}</option>
                            ))}
                        </select>
                    ) : ''}
                </div>
